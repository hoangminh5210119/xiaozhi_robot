...
I2CCommandBridge::I2CCommandBridge()
    : initialized_(false), owns_bus_handle_(false), bus_handle_(nullptr),
      dev_handle_(nullptr), polling_active_(false), polling_interval_ms_(1000),
      polling_task_handle_(nullptr), status_callback_(nullptr),
      callback_user_data_(nullptr) {}

I2CCommandBridge::~I2CCommandBridge() { Deinit(); }
...
bool I2CCommandBridge::Init() {
  ...
  initialized_ = true;
  ESP_LOGI(TAG,
           "✅ I2C Command Bridge initialized (SCL=%d, SDA=%d, Slave=0x%02X)",
           I2C_MASTER_SCL_IO, I2C_MASTER_SDA_IO, ACTUATOR_ESP32_ADDR);
  return true;
}

bool I2CCommandBridge::InitWithExistingBus(
    i2c_master_bus_handle_t existing_bus_handle) {
  if (initialized_) {
    ESP_LOGW(TAG, "Already initialized");
    return true;
  }
  ...
  esp_err_t err =
      i2c_master_bus_add_device(bus_handle_, &dev_config, &dev_handle_);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to add I2C device to existing bus: %s",
             esp_err_to_name(err));
    bus_handle_ = nullptr;
    return false;
  }
  ...
  initialized_ = true;
  ESP_LOGI(TAG,
           "✅ I2C Command Bridge initialized with shared bus (Slave=0x%02X)",
           ACTUATOR_ESP32_ADDR);
  return true;
}
...
std::string I2CCommandBridge::VehicleMoveTime(int direction, int speed_percent,
                                              int duration_ms) {
  if (!IsSlaveOnline()) {
  ...
  ESP_LOGI(TAG, "🚗 Vehicle: dir=%d speed=%d%% time=%dms → %s", direction,
           speed_percent, duration_ms, response.c_str());

  return response;
}

std::string I2CCommandBridge::VehicleMoveDistance(int direction,
                                                  int speed_percent,
                                                  int distance_mm) {
  if (!IsSlaveOnline()) {
    ESP_LOGW(TAG, "⚠️ Slave offline, skipping vehicle command");
  ...
  ESP_LOGI(TAG, "� Vehicle: dir=%d speed=%d%% dist=%dmm → %s", direction,
           speed_percent, distance_mm, response.c_str());
...
std::string I2CCommandBridge::StorageControl(int slot, int action) {
  ...
  ESP_LOGI(TAG, "� Storage: slot=%d action=%s → %s", slot,
           (action == ACT_OPEN ? "OPEN" : "CLOSE"), response.c_str());
...
std::string I2CCommandBridge::SendVehicleCommand(const std::string &direction,
...
std::string I2CCommandBridge::SendStorageCommand(int slot,
...
//     esp_err_t err = i2c_master_transmit(dev_handle_, (uint8_t*)jsonString,
//     jsonLen, TIMEOUT_MS); free(jsonString);
...
//     err = i2c_master_receive(dev_handle_, responseBuffer,
//     sizeof(responseBuffer), TIMEOUT_MS); if (err != ESP_OK) {
//         ESP_LOGW(TAG, "⚠️ Failed to receive response: %s",
//         esp_err_to_name(err)); return
//         "{\"status\":\"sent\",\"response\":\"timeout\"}";
//     }
...
std::string I2CCommandBridge::SendI2CCommand(cJSON *jsonCmd) {
  ...
  if (!jsonString)
    return "{\"error\":\"json_serialize_failed\"}";
  size_t jsonLen = strlen(jsonString);
  ESP_LOGI(TAG, "📤 Sending: %s (len=%d)", jsonString, (int)jsonLen);

  // ==== Send command ====
  const int TX_TIMEOUT_MS = 100;
  esp_err_t err = i2c_master_transmit(dev_handle_, (uint8_t *)jsonString,
                                      jsonLen, TX_TIMEOUT_MS);
  free(jsonString);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "⚠️ I2C transmit failed: %s", esp_err_to_name(err));
    return "{\"error\":\"i2c_send_failed\"}";
  }

  // ==== Wait for slave to respond ====
  vTaskDelay(pdMS_TO_TICKS(60));

  // ==== Read full response (chunked 32 bytes) ====
  std::string response;
  const int CHUNK_SIZE = 32;
  const int MAX_SIZE = 512; // bảo vệ chống loop vô tận
  const int RX_TIMEOUT = 100;

  while (true) {
    uint8_t buf[CHUNK_SIZE + 1] = {0};
    err = i2c_master_receive(dev_handle_, buf, CHUNK_SIZE, RX_TIMEOUT);

    if (err != ESP_OK) {
      if (err == ESP_ERR_TIMEOUT) {
        // Không còn dữ liệu → kết thúc
        break;
      } else {
        ESP_LOGW(TAG, "⚠️ Receive error: %s", esp_err_to_name(err));
        break;
      }
    }

    response.append((char *)buf, CHUNK_SIZE);

    // Dừng khi thấy dấu '}'
    if (response.find('}') != std::string::npos)
      break;

    if ((int)response.size() >= MAX_SIZE) {
      ESP_LOGW(TAG, "⚠️ Response too large (>512 bytes)");
      break;
    }
  ...
  size_t endPos = response.find_last_of('}');
...
void I2CCommandBridge::SetStatusCallback(ActuatorStatusCallback callback,
                                         void *user_data) {
...
bool I2CCommandBridge::StartStatusPolling(uint32_t interval_ms) {
  ...
  BaseType_t result = xTaskCreate(StatusPollingTask, "StatusPolling",
                                  4096, // Stack size
...
bool I2CCommandBridge::ParseStatusResponse(const std::string &jsonResponse,
                                           ActuatorStatus &status) {
  cJSON *root = cJSON_Parse(jsonResponse.c_str());
  if (!root) {
    ESP_LOGW(TAG, "⚠️ Failed to parse status JSON");
    return false;
  }

  // c = BLE connected state (0/1)
  cJSON *c = cJSON_GetObjectItem(root, "c");
  status.ble_connected = (c && c->valueint == 1);

  // h = heart rate (integer)
  cJSON *h = cJSON_GetObjectItem(root, "h");
  status.heart_rate = h ? h->valueint : -1;

  // g = servo states array [0,0,1,1]
  cJSON *g = cJSON_GetObjectItem(root, "g");
  if (g && cJSON_IsArray(g)) {
    int count = cJSON_GetArraySize(g);
    for (int i = 0; i < count && i < 4; i++) {
      cJSON *val = cJSON_GetArrayItem(g, i);
      status.storage[i].slot = i;
      status.storage[i].is_open = val && val->valueint == 1;
    }
  } else {
    for (int i = 0; i < 4; i++) {
      status.storage[i].slot = i;
      status.storage[i].is_open = false;
    }
  }

  ESP_LOGI(TAG, "📊 Parsed: BLE=%d, HR=%d, Servo=[%d,%d,%d,%d]",
           status.ble_connected, status.heart_rate, status.storage[0].is_open,
           status.storage[1].is_open, status.storage[2].is_open,
